...
public class Calc {
    ...
    public double performOperation(BinaryOperator<Double> op) throws IllegalStateException {
        if (getOperandCount() < 2) throw new IllegalStateException("Too few operands (" + getOperandCount() + ") on the stack");
     ...
     */
    public void swap() {
        if (getOperandCount() < 2) throw new IllegalStateException();
     ...
     */
    public void dup() {
        if (operandStack.isEmpty()) throw new IllegalStateException();
